...
**Accompanying implementation.** [vdom-harness](https://github.com/keejkrej/vdom-harness) is the runtime submitted with this paper (not a side project, not related-work-only). TypeScript virtual DOM: topology is a value; a reconciler mounts / updates / unmounts. This document does not clone that repo. Specified \(S\) is the catalog pointer of \(X=(H,M,E,C,S)\). Open [vdom-harness PR #13](https://github.com/keejkrej/vdom-harness/pull/13) stores a `CatalogPointer` `{ sku, servingPaused: false }` beside \(C\). That closes the `n.model` hole: \(I_{\mathrm{sku}}\) does not rewrite \(C\) topology; `n.model` stays 0731. This paper does **not** claim later serving is typed by \(S\), and does **not** claim the pointer is already \(X_n.S\) / `HybridState.S`. Remaining identification: `PhysicalNode.provider` is a process-global spray; \(S\) is a process pointer, not a per-episode coordinate. Papers, blogs, GitHub, X, and other agents are inputs to that runtime, not the product.
...
| \(S_n\) | \(\mathcal{S}\) | **slow** | *Catalog pointer* (SKU). Frozen on the fast clock. \(I_{\mathrm{sku}}\) gated-rebinds this specified coordinate only. Not \(f_\theta\). Price is not a coordinate of \(\mathcal{S}\). Runtime (open [vdom-harness PR #13](https://github.com/keejkrej/vdom-harness/pull/13)): `CatalogPointer` beside \(C\); `n.model` stays 0731. Remaining identification is `PhysicalNode.provider` as a global spray, not per-episode \(S\). `0813` is one available value, not the license. |
...
| `mount_sku` | \(S\) (slow) | Allowed only if the *eval / first-passage gate* passes (Definition 5.3 / Lemma 7.8). Specified write of catalog pointer \(S\). Runtime (open [vdom-harness PR #13](https://github.com/keejkrej/vdom-harness/pull/13)): `CatalogPointer` beside \(C\); `n.model` stays 0731. Do not claim later serving is typed by \(S\). “0813 exists” is not a gate (that is always-mount). |
...
2. **\(I_{\mathrm{sku}}\) writes catalog pointer \(S\).** Same \(C\). Specified gated rebind of \(S\). Runtime (open [vdom-harness PR #13](https://github.com/keejkrej/vdom-harness/pull/13)): `CatalogPointer` beside \(C\); \(I_{\mathrm{sku}}\) does not rewrite \(C\) topology; `n.model` stays 0731. Do not claim later serving is typed by \(S\). License: **incomplete episode (hang / no-write / crash).** Not price. Not \(f_\theta\). \(f_\theta\) / trainer / \(I_{\mathrm{weight}}\) is unimplemented. `0813` is one available SKU. SKU swap alone is not the contribution.
...
| catalog rebind (\(I_{\mathrm{sku}}\)) | *stand-in* slow cell: specified write of catalog pointer \(S\). Runtime (open [vdom-harness PR #13](https://github.com/keejkrej/vdom-harness/pull/13)): `CatalogPointer` beside \(C\); `n.model` stays 0731. Do not claim later serving is typed by \(S\). Not \(f_\theta\). Not the contribution. |
...
- [vdom-harness PR #13](https://github.com/keejkrej/vdom-harness/pull/13) (**open**): stores a `CatalogPointer` `{ sku, servingPaused: false }` beside \(C\). \(I_{\mathrm{sku}}\) writes that pointer; \(I_{\mathrm{loop}}\) writes \(C\); `n.model` stays 0731. That closes the `n.model` hole. Unit tests only. The mount is a unit fixture. Not a \(\tau^2\) result.

Remaining identification (not a Lemma 7.9 rewrite): `PhysicalNode.provider` is a global spray (`rebindServing` onto every key; `providerForNode` prefers bound over the pointer). \(S\) is a process pointer, not a per-episode coordinate of \(X\). This paper does **not** claim later serving is typed by \(S\), and does **not** claim \(S\) is already \(X_n.S\) / `HybridState.S`. A follow-up on #13 is the critic's test: after mixed fixture mount, `providerForNode(solve)` is 0731 for 39 and 0813 for 44. Gate is eval / first-passage, not “0813 exists.” \(f_\theta\) / trainer remains unimplemented. \(I_{\mathrm{sku}}\) is the stand-in slow cell. Do not sell \(p_{\mathrm{hit}}(0813)-p_{\mathrm{hit}}(0731)\). Do not put the pointer back inside the *specified* \(C\).
