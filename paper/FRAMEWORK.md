...
| \(S_n\) | \(\mathcal{S}\) | **slow** | *Catalog pointer* (SKU). Frozen on the fast clock. \(I_{\mathrm{sku}}\) gated-rebinds this specified coordinate only. Not \(f_\theta\). Price is not a coordinate of \(\mathcal{S}\). Runtime ([vdom-harness PR #13](https://github.com/keejkrej/vdom-harness/pull/13) / [#14](https://github.com/keejkrej/vdom-harness/pull/14), merged): `CatalogPointer` beside \(C\); `n.model` stays 0731. Later serving is typed by \(S\) for mixed 39/44. \(S\) is a per-episode controller coordinate (`ControlledEpisode.serving`). Not \(X_n.S\) / `HybridState.S`. Sidecar `servingByTask` is still a process table keyed by task. `0813` is one available value, not the license. |
...
| `mount_sku` | \(S\) (slow) | Allowed only if the *eval / first-passage gate* passes (Definition 5.3 / Lemma 7.8). Specified write of catalog pointer \(S\). Runtime ([vdom-harness PR #13](https://github.com/keejkrej/vdom-harness/pull/13) / [#14](https://github.com/keejkrej/vdom-harness/pull/14), merged): `CatalogPointer` beside \(C\); `n.model` stays 0731. Later serving is typed by \(S\) for mixed 39/44 (`providerForNode` prefers the per-task pointer). \(S\) is `ControlledEpisode.serving`, not `HybridState.S`. Sidecar `servingByTask` is a process table keyed by task. “0813 exists” is not a gate (that is always-mount). |
...
2. **\(I_{\mathrm{sku}}\) writes catalog pointer \(S\).** Same \(C\). Specified gated rebind of \(S\). Runtime ([vdom-harness PR #13](https://github.com/keejkrej/vdom-harness/pull/13) / [#14](https://github.com/keejkrej/vdom-harness/pull/14), merged): `CatalogPointer` beside \(C\); \(I_{\mathrm{sku}}\) does not rewrite \(C\) topology; `n.model` stays 0731. Later serving is typed by \(S\) for mixed 39/44. \(S\) is a per-episode controller coordinate (`ControlledEpisode.serving`), not `HybridState.S`. License: **incomplete episode (hang / no-write / crash).** Not price. Not \(f_\theta\). \(f_\theta\) / trainer / \(I_{\mathrm{weight}}\) is unimplemented. `0813` is one available SKU. SKU swap alone is not the contribution.
...
| catalog rebind (\(I_{\mathrm{sku}}\)) | *stand-in* slow cell: specified write of catalog pointer \(S\). Runtime ([vdom-harness PR #13](https://github.com/keejkrej/vdom-harness/pull/13) / [#14](https://github.com/keejkrej/vdom-harness/pull/14), merged): `CatalogPointer` beside \(C\); later serving typed by \(S\) for mixed 39/44. Per-episode controller coordinate, not `HybridState.S`. Not \(f_\theta\). Not the contribution. |
...
Lemma 7.9 is not a tomorrow item. License is incompleteness, not `loopExhausted`. Five citations, one sentence each:
...
- [vdom-harness PR #15](https://github.com/keejkrej/vdom-harness/pull/15) (merged): \(I_{\mathrm{sku}}\) ACCEPT protocol cell. Hung-44 traces; controller 44 \(I_{\mathrm{sku}}\), 39 \(I_{\mathrm{loop}}\), `waitKept=[]`. Fixture after (`after=1 > before=0`, labeled `incompleteFixture` / `notTau2Lift`). Mount: `controllerServing` `39.sku=0731 44.sku=0813`; \(C\) topology / `n.model` stay 0731. One live OpenRouter completion after mount recorded `servingModelAfter=deepseek/deepseek-v4-pro-0813`; `jumped=true`; `servingPaused=false`; `gate.action=mount`; `pHit0813=null`. Contrast #12 reject (omit after): `jumped=false`, serving stayed 0731. JSON: `eval/tau2/improve-live-0731-isku-44-mount.json`. Protocol cell, not a score. Not a Pro-vs-Flash lift. Not a \(\tau^2\) result. Live airline `improveLoop` still omits after. \(S\) is still a per-episode controller `CatalogPointer` (vdom #14), not `HybridState.S`. Trainer \(I_{\mathrm{weight}}\) remains unimplemented.

Remaining identification (not a Lemma 7.9 rewrite): this paper does **not** claim \(S\) is already \(X_n.S\) / `HybridState.S`. #13 closed `n.model`; #14 closed process `servingSku` as the lookup. #15 is the \(I_{\mathrm{sku}}\) ACCEPT protocol cell, not `HybridState.S` and not a Pro-vs-Flash score. Remaining identification is sidecar `servingByTask` as a process table keyed by task, not `HybridState.S`. If a caller omits the SKU argument, bound / `n.model` still win; official later-serving helpers must keep passing \(S\). Gate is eval / first-passage, not “0813 exists.” \(f_\theta\) / trainer remains unimplemented. \(I_{\mathrm{sku}}\) is the stand-in slow cell. Do not sell \(p_{\mathrm{hit}}(0813)-p_{\mathrm{hit}}(0731)\). Do not put the pointer back inside the *specified* \(C\). Rating is not a SOTA claim.
