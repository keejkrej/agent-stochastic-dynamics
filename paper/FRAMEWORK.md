...
**Accompanying implementation.** [vdom-harness](https://github.com/keejkrej/vdom-harness) is the runtime submitted with this paper (not a side project, not related-work-only). TypeScript virtual DOM: topology is a value; a reconciler mounts / updates / unmounts. This document does not clone that repo. Specified \(S\) is the catalog pointer of \(X=(H,M,E,C,S)\). [vdom-harness PR #11](https://github.com/keejkrej/vdom-harness/pull/11) logs typed \(\mathrm{Obs}\) hung-first. [vdom-harness PR #12](https://github.com/keejkrej/vdom-harness/pull/12) is the live hung-44 reject cell. [vdom-harness PR #13](https://github.com/keejkrej/vdom-harness/pull/13) (merged) stores a `CatalogPointer` beside \(C\); \(I_{\mathrm{sku}}\) does not rewrite \(C\) topology; `n.model` stays 0731; later serving is typed by \(S\). [vdom-harness PR #14](https://github.com/keejkrej/vdom-harness/pull/14) (merged) makes \(S\) a per-episode controller coordinate. [vdom-harness PR #15](https://github.com/keejkrej/vdom-harness/pull/15) (merged) is the \(I_{\mathrm{sku}}\) ACCEPT protocol cell. [vdom-harness PR #16](https://github.com/keejkrej/vdom-harness/pull/16) (merged) dumps controller \(X_n.S\) after that licensed write. [vdom-harness PR #17](https://github.com/keejkrej/vdom-harness/pull/17) (merged) writes greeting-turn \(H/M\) onto the same \(X\). [vdom-harness PR #18](https://github.com/keejkrej/vdom-harness/pull/18) (merged) is dump-label honesty. [vdom-harness PR #19](https://github.com/keejkrej/vdom-harness/pull/19) (merged) writes greeting-turn serving-step \(E\) onto the HybridState the turn used; the honesty lock is §9. Papers, blogs, GitHub, X, and other agents are inputs to that runtime, not the product.
...
| `mount_sku` | \(S\) (slow) | Allowed only if the *eval / first-passage gate* passes (Definition 5.3 / Lemma 7.8). Specified write of catalog pointer \(S\). Same \(C\); `n.model` stays 0731; later serving is typed by \(S\). The runtime log is §9. “0813 exists” is not a gate (that is always-mount). |
...
2. **\(I_{\mathrm{sku}}\) writes catalog pointer \(S\).** Same \(C\). Specified gated rebind of \(S\). The controller log is §9. License: **incomplete episode (hang / no-write / crash).** Not price. Not \(f_\theta\). \(f_\theta\) / trainer / \(I_{\mathrm{weight}}\) is unimplemented. `0813` is one available SKU. SKU swap alone is not the contribution.
...
| catalog rebind (\(I_{\mathrm{sku}}\)) | *stand-in* slow cell: specified write of catalog pointer \(S\). Same \(C\); later serving typed by \(S\). The runtime log is §9. Not \(f_\theta\). Not the contribution. |
...
While a slow request is in flight, the fast process remains Markov with kernel \(K_{C,S}\) for the *pre-request* pair \((C,S)\). \(S\) is frozen. `servingPaused=false`. Arrival is an optional exogenous time \(T_{\mathrm{adapt}}\). On \(\{T_{\mathrm{adapt}}=n\}\cap\{g=\mathrm{mount}\}\) mount writes catalog pointer \(S\) and the kernel switches to \(K_{C,S'}\); otherwise it does not. Mount does not rebind `n.model` and does not spray a global `PhysicalNode.provider`. Jump iff later serving uses \(S'\). Official later-serving helpers must pass \(S\); if a caller omits the SKU argument, bound / `n.model` still win. The runtime log of this write is §9. This is a piecewise-homogeneous hybrid Markov process. It is not a diffusion in \(\theta\). It is not SGD. It is not a trainer job. On this stack the available actuator is \(I_{\mathrm{sku}}\) (0731 \(\to\) 0813), a cell.
...
**Controller log (not a slogan).** [vdom-harness PR #11](https://github.com/keejkrej/vdom-harness/pull/11) implements the map hung-first: 39 \(I_{\mathrm{loop}}\) / 44 \(I_{\mathrm{sku}}\) / `waitKept` empty. [vdom-harness PR #12](https://github.com/keejkrej/vdom-harness/pull/12) is the live hung-44 reject cell. [vdom-harness PR #13](https://github.com/keejkrej/vdom-harness/pull/13) (merged) stores a `CatalogPointer` beside \(C\). [vdom-harness PR #14](https://github.com/keejkrej/vdom-harness/pull/14) (merged) makes \(S\) a per-episode controller coordinate. [vdom-harness PR #15](https://github.com/keejkrej/vdom-harness/pull/15) (merged) is the \(I_{\mathrm{sku}}\) ACCEPT protocol cell. [vdom-harness PR #16](https://github.com/keejkrej/vdom-harness/pull/16) (merged) dumps controller \(X_n.S\). [vdom-harness PR #17](https://github.com/keejkrej/vdom-harness/pull/17) (merged) writes greeting-turn \(H/M\) on the same \(X\). [vdom-harness PR #18](https://github.com/keejkrej/vdom-harness/pull/18) (merged) is dump-label honesty. [vdom-harness PR #19](https://github.com/keejkrej/vdom-harness/pull/19) (merged) writes greeting-turn serving-step \(E\) onto the HybridState the turn used. The honesty lock is §9.
...
Lemma 7.9 is not a tomorrow item. License is incompleteness, not `loopExhausted`. Nine citations, one sentence each:
...
- [vdom-harness PR #13](https://github.com/keejkrej/vdom-harness/pull/13) (merged): stores a `CatalogPointer` `{ sku, servingPaused: false }` beside \(C\); \(I_{\mathrm{sku}}\) writes that pointer; later serving is typed by \(S\) for mixed 39/44.
- [vdom-harness PR #14](https://github.com/keejkrej/vdom-harness/pull/14) (merged): \(S\) is a per-episode controller coordinate (`ControlledEpisode.serving`); a fresh 39-only batch after mixed mount stays 0731.
- [vdom-harness PR #15](https://github.com/keejkrej/vdom-harness/pull/15) (merged): \(I_{\mathrm{sku}}\) ACCEPT protocol cell; fixture after + live 0813 serve recorded the serving id.
- [vdom-harness PR #16](https://github.com/keejkrej/vdom-harness/pull/16) (merged, `1bbda40`): controller \(X_n.S\) dump after licensed write; reconstructed hung obs, `H=[]` `M=[]`.
- [vdom-harness PR #17](https://github.com/keejkrej/vdom-harness/pull/17) (merged, `cc9d812`): greeting-turn \(H/M\) on the same HybridState after licensed write; incoming messages empty.
- [vdom-harness PR #18](https://github.com/keejkrej/vdom-harness/pull/18) (merged, `fe33ad2`): dump-label honesty on that same JSON; `licenseE` and `servingE` are two named facts.
- [vdom-harness PR #19](https://github.com/keejkrej/vdom-harness/pull/19) (merged, `4aaf123`): writes greeting-turn serving-step \(E\) onto the same HybridState the turn mutated; dump serializes `X.E` / `X.licenseE` from that object.

**Honesty lock.** Paper \(X=(H,M,E,C,S)\). licenseE is a fixture field beside paper \(X\), not paper \(E\), not the \(E\) factor of the hybrid process, not a sixth paper coordinate. Do not write licenseE is \(E\). After #19, runtime \(X.E\) / `servingE` is greeting-turn serving-step \(E\) (`hung=false`). Not a \(\tau^2\) user/gym step. Not live hung-44 then served. #19 is not a result. Fresh hang then \(I_{\mathrm{sku}}\) is an open identification hole. Do not fill it with a score or by turning on `after=`. No \(p_{\mathrm{hit}}(0813)\). Trainer \(I_{\mathrm{weight}}\) remains unimplemented. \(I_{\mathrm{sku}}\) is the stand-in cell.
