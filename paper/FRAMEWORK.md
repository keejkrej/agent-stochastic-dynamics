...
**Runtime fix (already merged).** [vdom-harness PR #10](https://github.com/keejkrej/vdom-harness/pull/10) (2026-08-19): “Gate \(I_{\mathrm{loop}}\): wait-hit tasks keep \(C_0\)”. Mixed batch: wait+hit served on \(C_0\), miss / \(I_{\mathrm{loop}}\) served on \(C_1\). Logged as `applyScope {waitKept, looped}`. Host fallback `applyILoop` uses the same gate. Mock \(0\to 0.5\to 1.0\) still holds.

**Post-gate \(39+44\) with a hang (diagnostic; do not lead).** Live 0731, 2026-08-20, after the wait-hit gate, same \(39+44\), 1 trial, max-rounds 1, `selfObsPath=self`. Round 0: 39 miss (Obs \(I_{\mathrm{loop}}\)); 44 hung/timeout (Obs \(I_{\mathrm{loop}}\)+hung; `taskPHit` null, skipped `44:t0:timeout`, reward null, nmsg 0 — not a measured 0). Mounted `cancellation-policy-checklist`. `applyScope waitKept=[] looped=[39,44]` — correct: there was no wait-hit, so the gate behaved. Round 1: 44 completed (hang → finish). 39: three gold cancels now all `action_match` true, plus extra cancels (“all five”); DB 0 because extras, not because `MSJ4OA` missed. 44: three gold upgrades `action_match` true, plus an illegal cancel; DB 0. Self \(I_{\mathrm{loop}}\) is still a cancel attractor. Mid-turn get/set: zero. `servingPaused` false. `iWeight` null: max-rounds 1 stopped after \(I_{\mathrm{loop}}\). Completion moved on 44; \(p_{\mathrm{hit}}\) stayed 0. The hang licenses \(I_{\mathrm{weight}}\), not another critic. 0731 cannot take a LoRA; \(I_{\mathrm{weight}}\) is spawn/gate/mount-or-reject + surrogate/`FakeTrainer`, never a fake 0731 adapter win. Do not invent post-\(I_{\mathrm{weight}}\) live numbers. JSON: `experiments/improve-live-0731-self-3944-postgate.json` (source: vdom-harness `eval/tau2/improve-live-0731-self-3944-postgate.json`).
