...
Self-obs on airline \(39+44\) (live 0731, 2026-08-19, 1 trial, max-rounds 1, `selfObsPath=self`): \(p_{\mathrm{hit}}\) \(0.5\to 0.0\) (`experiments/improve-live-0731-self-3944.json`). Round 0: 39 miss (Obs \(I_{\mathrm{loop}}\)); 44 hit (Obs `wait`). Round 1: 39 still miss; 44 regressed \(1\to 0\) after a global `cancel_policy`. Mid-turn get/set: zero calls. `servingPaused` false. Not \(I_{\mathrm{weight}}\). Same shape as held-out 18: wait-hit still got a global \(C\) change — an illegal apply, not a SOTA miss. Evidence the loop ran and Obs chose \(I_{\mathrm{loop}}\). Do not lead with \(0.5\to 0.0\). Runtime fix: [vdom-harness PR #10](https://github.com/keejkrej/vdom-harness/pull/10) (merged 2026-08-19) scopes apply — wait-hit keeps \(C_0\). Recorded in SDS #8.

Post-gate mixed batch with a hang (live 0731, 2026-08-20, after the wait-hit gate, same \(39+44\), 1 trial, max-rounds 1, `selfObsPath=self`): `experiments/improve-live-0731-self-3944-postgate.json`. Do not lead with \(0\to 0\). Round 0: 39 miss (Obs \(I_{\mathrm{loop}}\)); 44 hung/timeout (Obs \(I_{\mathrm{loop}}\)+hung; skipped `44:t0:timeout`, reward null — not a measured 0). Mounted `cancellation-policy-checklist`. `applyScope waitKept=[] looped=[39,44]` — correct: no wait-hit, so the gate behaved. Round 1: 44 completed (hang → finish). 39: three gold cancels all `action_match` true plus extras (“all five”); DB 0 because extras. 44: three gold upgrades `action_match` true plus an illegal cancel; DB 0. Self \(I_{\mathrm{loop}}\) still a cancel attractor. Mid-turn get/set: zero. `servingPaused` false. `iWeight` null (max-rounds 1 stopped after \(I_{\mathrm{loop}}\)). Completion moved on 44; \(p_{\mathrm{hit}}\) stayed 0. The hang licenses \(I_{\mathrm{weight}}\), not another critic. 0731 cannot take a LoRA; \(I_{\mathrm{weight}}\) is spawn/gate/mount-or-reject + surrogate/`FakeTrainer`, never a fake 0731 adapter win. Do not invent post-\(I_{\mathrm{weight}}\) live numbers.
