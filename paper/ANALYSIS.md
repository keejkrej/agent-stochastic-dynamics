...
**Runtime fix (already merged).** [vdom-harness PR #10](https://github.com/keejkrej/vdom-harness/pull/10), merged 2026-08-19: “Gate $I_{\mathrm{loop}}$: wait-hit tasks keep $C_0$”. Mixed batch: wait+hit served on $C_0$, miss / $I_{\mathrm{loop}}$ served on $C_1$. Logged as `applyScope {waitKept, looped}`. Host fallback `applyILoop` uses the same gate. Mock $0\to 0.5\to 1.0$ still holds. A later live 0731 mixed batch after the gate is §0a3.

Still missing on *this* pre-gate run: serving mid-turn get/set was never called; 39’s remaining miss (`MSJ4OA`) is not this PR; $I_{\mathrm{weight}}$ is not licensed (episodes completed).

### 0a3. Post-gate $39+44$ with a hang: apply scoped; hang licenses $I_{\mathrm{weight}}$ (do not lead)

Not the eval. Not a failed SOTA attempt. Do not lead with $0\to 0$. Keep §0c as the locked reading of the held-out $0.7\to 0.9$.

Live 0731, 2026-08-20, airline $39+44$, 1 trial, max-rounds 1, `selfObsPath=self`, **after** the wait-hit gate. JSON: `experiments/improve-live-0731-self-3944-postgate.json` (source: vdom-harness `eval/tau2/improve-live-0731-self-3944-postgate.json`). `servingPaused` false. Mid-turn `get_agent_graph` / `set_agent_graph`: **zero** calls. `iWeight` null (max-rounds 1 stopped after $I_{\mathrm{loop}}$).

| Cycle $t$ | $\mathrm{Obs}(\mathrm{traces}_t)$ / completion | $P^{\mathrm{ctrl}}$ | $\mathrm{pass}^1(t)$ |
| --- | --- | --- | --- |
| $0$ | 39 miss (`I_loop`); 44 hung/timeout (`I_loop`+hung; skipped, not a measured 0) | — | $0.0$ (completed only) |
| $1$ | 44 completed (hung → finish miss); cancel attractor on both | $I_{\mathrm{loop}}$ (`cancellation-policy-checklist`) | $0.0$ |

- **Task 39.** Round 0: cancel `8C8K4E`+`LU15PA` matched; `MSJ4OA` `action_match` false. Round 1: the three gold cancels now all `action_match` true, plus extra cancels (“all five”). DB 0 because extras, not because `MSJ4OA` missed.
- **Task 44.** Round 0: hung/timeout (`taskPHit` null, skipped `44:t0:timeout`, reward null, nmsg 0). Not a measured 0. Round 1: completed. Three gold upgrades `action_match` true, **plus an illegal cancel**. DB 0.

`applyScope waitKept=[] looped=[39,44]` — **correct** under the gate: there was no wait-hit, so both tasks received $C_1$. The gate behaved.

Self $I_{\mathrm{loop}}$ is still a cancel attractor: 44 finished, then cancelled something it should not. Completion moved on 44; $p_{\mathrm{hit}}$ stayed 0. The hang licenses $I_{\mathrm{weight}}$, not another critic. 0731 cannot take a LoRA; $I_{\mathrm{weight}}$ is spawn/gate/mount-or-reject + surrogate/`FakeTrainer`, never a fake 0731 adapter win. **Do not invent post-$I_{\mathrm{weight}}$ live numbers.**

Still missing: serving mid-turn get/set was never called; $I_{\mathrm{weight}}$ is licensed but not taken.
