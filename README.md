...
A serving agent is a controlled hybrid Markov process \(X=(H,M,E,C)\) with kernel \(K_C\). \(C\) includes the bound model pointer (SKU). Serve the cheapest capable model as the base SKU. After \(\mathrm{Obs}\): \(I_{\mathrm{loop}}\) (mutate the graph / \(C\), same SKU, fast clock, serving does not pause) or \(I_{\mathrm{sku}}\) (request a more expensive released checkpoint, slow clock, serving continues on the cheap SKU, gated rebind of `PhysicalNode.provider` / `n.model`). Wait is the identity. Incomplete (hang / no-write / crash) \(\to I_{\mathrm{sku}}\). Completed miss + attractor \(\to I_{\mathrm{loop}}\). Hits wait. Extra \(H\) is neither arm. Concrete cell: base `deepseek/deepseek-v4-flash-0731`, escalate `deepseek/deepseek-v4-pro-0813`. Trainer-spawn is related work, not the claim. Not \(\tau^2\) SOTA. Not \(p_{\mathrm{hit}}(0813)-p_{\mathrm{hit}}(0731)\) as a result. See `paper/FRAMEWORK.md`.
