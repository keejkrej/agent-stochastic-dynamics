...
A serving agent is a controlled hybrid Markov process \(X=(H,M,E,C,S)\) with kernel \(K_{C,S}\). \(C\) is the fast graph; \(S\) is the slow decoder pointer. Self-\(\mathrm{Obs}\) types the failure and chooses which coordinate to edit. Two licensed edits: \(I_{\mathrm{loop}}\) mutates \(C\) (same \(S\)); the slow arm gated-rebinds \(S\) (same \(C\)). Wait is the identity. License for the slow arm is an incomplete episode (hang / no-write / crash), not a more expensive checkpoint. Extra \(H\) is neither arm. Catalog rebind \(I_{\mathrm{sku}}\) (`0731` \(\to\) `0813`) is the implemented slow actuator — a cell, not the title claim. SKU swap alone is not novel. Do not sell \(p_{\mathrm{hit}}(0813)-p_{\mathrm{hit}}(0731)\). Do not title this as model routing. See `paper/FRAMEWORK.md`.
