...
A serving agent is a controlled hybrid Markov process \(X=(H,M,E,C)\) with kernel \(K_C\). Self-\(\mathrm{Obs}\) types the failure and chooses which factor to intervene on: \(I_{\mathrm{loop}}\) mutates \(C\) on the fast clock (same SKU); the slow clock is a gated jump of \(f_\theta\), serving unpaused. Wait is the identity. Incomplete \(\to\) slow \(f_\theta\) actuator; completed miss + attractor \(\to I_{\mathrm{loop}}\); hits wait. Extra \(H\) is neither arm. The available slow actuator *on this stack* is a catalog rebind \(I_{\mathrm{sku}}\) (`0731` \(\to\) `0813`) because we cannot train — a cell, not the claim. SKU swap alone is not novel (FrugalGPT, RouteLLM, OpenRouter fallbacks). Trainer-spawn is unimplemented. Do not sell \(p_{\mathrm{hit}}(0813)-p_{\mathrm{hit}}(0731)\). Do not title this as model routing. See `paper/FRAMEWORK.md`.
